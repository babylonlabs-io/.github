...
- reusable_docker_pipeline: **breaking** — `dockerContext` and `dockerfile` must be relative paths inside the checked-out repository (`[A-Za-z0-9_./-]`, no `..`, no leading `-` or `/`, no `github.com/...`); URLs, git refs and other remote contexts are rejected (baby-auditor-infra-findings#119). A remote context was fetched separately by the amd64 and arm64 jobs, so one tag could combine two revisions that are not the reviewed commit. Both values are validated in `prepare-metadata`, passed on as normalized `./path` outputs, and `docker_build` checks after the checkout that the context and the Dockerfile's directory do not resolve (through symlinks) outside the workspace. No current caller passes a remote context; an explicitly empty `dockerContext: ''` (previously the action's git context) is rejected too.
