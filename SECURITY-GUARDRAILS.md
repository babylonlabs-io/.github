...
Scope: this is input validation only. It keeps one caller's tag from becoming another's; where the per-platform images live is the next rule.

### Never publish a per-platform image under a name a caller can select
Always: the per-platform images `docker_build` pushes are staging artifacts of one run, not advertised names. They are published as `<validated tag>-r<run id>.<run attempt>-linux-<arch>`, composed in `prepare-metadata` (fixed runner, `permissions: {}`, no secrets) and passed on as the `platform-tag-prefix` output; `docker_build` and both merge jobs append the platform pair to that output and never build a platform reference from the final tag. Two things make the staging namespace unreachable: every platform suffix the matrix can produce is reserved by the tag validator, so no final tag of any invocation can name a platform tag, and the run scope comes from `GITHUB_RUN_ID`/`GITHUB_RUN_ATTEMPT`, so no two publications (or re-runs) share a staging tag. Both halves are load-bearing — a new matrix platform must be added to `PLATFORM_SUFFIXES` in the same commit, and the composed name must still fit the 128-character registry limit (the validator fails closed when it does not). `repoName` must be a single registry repository name publishable in both registries — `^[a-z0-9]+([._-][a-z0-9]+)*$`, 2–128 characters, the intersection of the Docker Hub (OCI reference) and ECR grammars: no uppercase, no `__` / `--`, no `/`, `:` or `@`, so the reference cannot be pointed at another namespace, tag or digest and a name the registry would refuse fails before any credentials exist. The fallback (this repository's name) is validated the same way.
Bad:  `docker push "$REPO:${IMAGE_TAG}-${PLATFORM_PAIR}"`; `imagetools create -t "$first_tag" "$REPO:${IMAGE_TAG}-linux-amd64"`; adding `linux/riscv64` to the matrix without reserving `-linux-riscv64`.
Good: `TAG="${PLATFORM_TAG_PREFIX}-${PLATFORM_PAIR}"` in the push steps and `"$REPO:${PLATFORM_TAG_PREFIX}-{}"` in the merge jobs, with `PLATFORM_TAG_PREFIX` bound from `needs.prepare-metadata.outputs.platform-tag-prefix`.
Enforced by: `.github/workflows/test_ecr_tag_collisions.yml` runs `tests/test_docker_pipeline_tag_context.py`, which derives the platform pairs from the real matrix and `Prepare` steps, feeds every platform tag this workflow can push back through the real tag validator (the [baby-auditor-infra-findings#120](https://github.com/babylonlabs-io/baby-auditor-infra-findings/issues/120) payload `release-linux-amd64`, its arm64 twin and the run-scoped names), checks that staging tags are unique per run and per tag, and executes the real push and merge blocks.
Scope: this removes the shared platform-tag namespace. The merge jobs still reference the staging images by tag rather than by the digests recorded during the run (#47), so a direct holder of the registry credentials — not another invocation of this workflow — could still replace a staging image between the push and the merge. Docker Hub tags remain mutable: republishing an existing final tag with different bytes is still possible for an authorized caller.
